...
/*   Updated: 2024/07/16 17:55:06 by aisidore         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (ft_free(&basin));
	block = read_one_block(fd);
	if (block == NULL && ft_strchr(basin, '\n') == NULL)
	{
		last_line = ft_dup(basin);
		if (basin != NULL)
			ft_free(&basin);
		return (last_line);
	}
	basin = ft_join_free(basin, block);
	if (ft_strchr(basin, '\n'))
		return (split_basin_line(&basin));
	else
